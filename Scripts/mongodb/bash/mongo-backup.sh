...
#send_mattermost_notification ":red_circle: This is a test message! @all"
...
perform_full_backup() {
   ...
   #    send_mattermost_notification "❌ MongoDB Full  Backup Failed - ${TIMESTAMP}"
   #   return 1
   # fi
    ...
    # Step 4: Backup just the oplog entries between start and end
cat > /tmp/oplog_query.js << EOF
{
  "ts": {
    "\$gte": {
      "\$timestamp": {
        "t": $(echo "$OPLOG_START" | grep -oP '(?<=t: )\d+'),
        "i": $(echo "$OPLOG_START" | grep -oP '(?<=i: )\d+')
      }
    },
    "\$lte": {
      "\$timestamp": {
        "t": $(echo "$OPLOG_END" | grep -oP '(?<=t: )\d+'),
        "i": $(echo "$OPLOG_END" | grep -oP '(?<=i: )\d+')
      }
    }
  }
}
EOF
   ...
   # mongodump --uri="$MONGODB_URI" --archive="$BACKUP_DIR/oplog_$TIMESTAMP.gz" --gzip \
   #   --db=local --collection=oplog.rs --queryFile=/tmp/oplog_query.js 2>&1 | tee -a "$BACKUP_LOG"
   ...
   # mc cp "/$BACKUP_DIR/oplog_${TIMESTAMP}.gz" "myminio/${BUCKET_NAME}/oplog/" 2>&1 | tee -a "$BACKUP_LOG"
   # mc ilm rule add "myminio/${BUCKET_NAME}/oplog/oplog_${TIMESTAMP}.gz" --expire-days "7"
   # mc cp "/$BACKUP_DIR/oplog_position.txt" "myminio/${BUCKET_NAME}/oplog/" 2>&1 | tee -a "$BACKUP_LOG"
...
perform_incremental_backup() {
   ...
   # echo "$CURRENT_OPLOG" > "$BACKUP_DIR/oplog_position.txt"
    ...
    #QUERY= "{\"ts\":{\"\$gt\": $LAST_OPLOG_TS_EXTENDED_JSON}}"
...
EOF
    ...
    cat /tmp/query.js
   # mongodump --uri="$MONGODB_URI" --archive="/$BACKUP_DIR/oplog_${TIMESTAMP}.gz --gzip" \
   #     --db=local --collection=oplog.rs --queryFile=/tmp/query.js  2>&1 | tee -a "$BACKUP_LOG"
    ...
    mc ilm rule add "myminio/${BUCKET_NAME}/oplog/oplog_${TIMESTAMP}" --expire-days "7"
    mc cp "/$BACKUP_DIR/oplog_position.txt" "myminio/${BUCKET_NAME}/oplog/"
